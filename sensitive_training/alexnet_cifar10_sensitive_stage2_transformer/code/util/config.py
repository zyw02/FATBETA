...
def init_logger(experiment_name, output_dir, cfg_file=None):
    ...
    if cfg_file is not None and os.path.exists(cfg_file):
        try:
            ...
            # Fallback to basic config if fileConfig fails
            print(f"Warning: Failed to load logging config from {cfg_file}: {e}")
            print("Using basic logging configuration instead.")
            logging.basicConfig(
                level=logging.INFO,
        ...
        # Use basic config if no config file provided
        logging.basicConfig(
            level=logging.INFO,
            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
            handlers=[
