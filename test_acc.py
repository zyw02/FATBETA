...
def check_sd_match(model, checkpoint_sd):
    ...
    for k, v in model_sd.items():
        if k in checkpoint_sd:
            # Assuming there was an implicit condition here, or it was meant to be unconditionally matched if key exists
            if v.shape == checkpoint_sd[k].shape: # Added this condition for syntactic correctness and common logic
                matched += 1
            else:
                print(f"Shape mismatch: {k} (Model: {v.shape}, Ckpt: {checkpoint_sd[k].shape})") # Corrected ckpt_sd to checkpoint_sd
                mismatched += 1
        else:
            # print(f"Missing in ckpt: {k}")
            missing_in_ckpt += 1
            
    return matched, mismatched, missing_in_ckpt
...
def main():
    try:
        ...
        # Define bits and get quantized layers for EMA
        bits = 8
        q_layers_ema, _ = get_quantized_layers(model)
        
        load_checkpoint(model, ckpt_path, model_device=device, use_ema=True)
        print("EMA weights loaded.")
        
        set_bit_width(model, [bits]*len(q_layers_ema), [bits]*len(q_layers_ema))
        
        # Store EMA weights for comparison
        import copy
        ema_state_dict = copy.deepcopy(model.state_dict())
        
        match_count, mismatch_count, missing_count = check_sd_match(model.state_dict(), checkpoint['state_dict_ema'])
        print(f"Match Stats (EMA vs Checkpoint): Matched={match_count}, Mismatched={mismatch_count}, MissingInCkpt={missing_count}")
        ...
        # Compare weights
        print("\n[DEBUG] Weight Difference Check:")
        diff_norm = 0.0
        std_state_dict = model.state_dict()
        for k in ema_state_dict:
            if k in std_state_dict:
                d = (ema_state_dict[k] - std_state_dict[k]).float().norm().item()
                diff_norm += d
        print(f"Total L2 Difference between EMA and Standard weights: {diff_norm:.4f}")
