...
def run_experiment(base_cfg_path, ber, ber_base, ber_msb):
    ...
    exp_name = f"r20_b{ber:.4f}_base{ber_base:.4f}_msb{ber_msb:.4f}".replace('0.', '.')
    cfg['name'] = exp_name
    
    # 4. 创建临时配置文件
    tmp_config_dir = Path("scripts/tmp_configs")
    ...
    if log_file.exists():
        print(f"⏩ [SKIP] Experiment {exp_name} already exists. Skipping...")
        return
    ...
    print(f"🚀 [START] Running experiment: {exp_name}")
    ...
    try:
        ...
        if checkpoint_path.exists():
            print(f"🔍 [SWEEP] Found checkpoint, starting fault injection sweep...")
            sweep_log_path = (Path.cwd() / cfg.get('output_dir', 'training') / exp_name / "fast_sweep_results.log").resolve()
            
            # 2. 构造 sweep 命令 (去掉了 --config，直接使用脚本内默认配置，并禁用颜色输出)
            sweep_cmd = [
            ...
            with open(sweep_log_path, "w") as sweep_f:
                ...
                # 执行脚本，stdout 丢进文件，stderr 打印到控制台
                subprocess.run(sweep_cmd, stdout=sweep_f, stderr=subprocess.STDOUT, check=True)
...
def main():
    base_config = "configs/training/r20_2.yaml"
    
    # 定义搜索空间 (你可以根据需要调整这里的列表)
    # 区间 [0.001, 0.02]
    # 如果做全网格搜索，点数不宜过多，否则耗时极长
    bers = [0.001, 0.005, 0.01, 0.02]
    ber_bases = [0.001, 0.005, 0.01, 0.02]
    ber_msbs = [0.001, 0.005, 0.01, 0.02]
