        ...
        if self.mode == "ber":
            ...
            if layer_name is not None and self.use_position_based_mask:
                ...
                if self.bfat_dual_bit:
                    ...
                    # All bits: if ber_msb is provided, override MSB with it
                    if self.ber_msb is not None:
                        p_msb = self.ber_msb
                        if layer_name is not None and self.use_position_based_mask:
                            # Re-generate MSB mask with p_msb for hash-based
                            hash_grid = hash_tensor.reshape(N, k)
                            mask[:, k-1] = hash_grid[:, k-1] < p_msb
                        else:
                            # Re-generate MSB mask with p_msb for random-based
                            if seed_to_use is not None:
                                gen = torch.Generator(device=device)
                                gen.manual_seed(seed_to_use + 999) # Use different seed for independence if needed
                                mask[:, k-1] = torch.rand(N, generator=gen, device=device) < p_msb
                            else:
                                mask[:, k-1] = torch.rand(N, device=device) < p_msb
                ...
                if self.bfat_dual_bit:
                    ...
                    if self.ber_msb is not None:
                        p_msb = self.ber_msb
                        # Already generated by torch.rand < p, now override MSB
                        if seed_to_use is not None:
                            gen = torch.Generator(device=device)
                            gen.manual_seed(seed_to_use + 888)
                            mask[:, k-1] = torch.rand(N, generator=gen, device=device) < p_msb
                        else:
                            mask[:, k-1] = torch.rand(N, device=device) < p_msb
