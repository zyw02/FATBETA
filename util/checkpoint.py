...
from quan import QuanConv2d, SwithableBatchNorm
...
def load_checkpoint(model:nn.Module, chkp_file, model_device=None, strict=True, lean=False, optimizer=None, override_optim=False, lr_scheduler=None, lr_scheduler_q=None, optimizer_q=None, output_corrector=None, corrector_optimizer=None, sensitive_restorer=None, sensitive_optimizer=None, learnable_olm_manager=None, olm_optimizer=None, search_olm_manager=None, use_ema=False):
    ...
    Args:
        ...
        use_ema: if True, load weights from 'state_dict_ema' instead of 'state_dict'
    ...
    state_dict_key = 'state_dict_ema' if use_ema else 'state_dict'
    if state_dict_key not in checkpoint or checkpoint[state_dict_key] is None:
        if use_ema:
            logger.warning(f"EMA state dict not found in {chkp_file}, falling back to standard state_dict")
            state_dict_key = 'state_dict'
        else:
            raise ValueError(f'Checkpoint must contain {state_dict_key}')

    if state_dict_key not in checkpoint:
        raise ValueError(f'Checkpoint must contain model parameters ({state_dict_key})')
    ...
    if optimizer_q is not None and optimizer_q_state is not None and not override_optim and not lean:
        # Filter optimizer state to remove parameters with shape mismatch
    ...
    if lr_scheduler is not None and lr_scheduler_state is not None and not lean:
        lr_scheduler.load_state_dict(lr_scheduler_state)

    lr_scheduler_q_state = checkpoint.get('lr_scheduler_q', None)
    if lr_scheduler_q is not None and lr_scheduler_q_state is not None and not lean:
    ...
    # Pre-process checkpoint to handle SwithableBatchNorm mapping
    # If a layer in the checkpoint (e.g. features.0.1) is a standard BN,
    # but in the model it's a SwithableBatchNorm (features.0.1.bn_list.0.0...),
    # we distribute the weights to all sub-BNs.
    expanded_checkpoint_state = {}
    for key, value in checkpoint[state_dict_key].items():
        # Check if this key corresponds to a standard BN parameter that might be in a SwithableBatchNorm
        # keys like 'features.0.1.weight', 'features.0.1.bias', 'features.0.1.running_mean', etc.
        # OR 'features.0.1.bn.weight', 'features.0.1.bn.bias', etc.
        parts = key.split('.')
        if len(parts) >= 1:
            base_name = '.'.join(parts[:-1])
            param_name = parts[-1]
            
            # Check if base_name is a SwithableBatchNorm in the model
            module = None
            try:
                module = unwrapped_model.get_submodule(base_name)
            except Exception:
                pass
            
            # If not found or not SwithableBatchNorm, check if it's base_name.bn.param_name
            if (module is None or not isinstance(module, SwithableBatchNorm)) and base_name.endswith('.bn'):
                parent_name = base_name[:-3]
                try:
                    parent_module = unwrapped_model.get_submodule(parent_name)
                    if isinstance(parent_module, SwithableBatchNorm):
                        module = parent_module
                        base_name = parent_name
                except Exception:
                    pass
            
            if isinstance(module, SwithableBatchNorm) and hasattr(module, 'bn_list'):
                # Map this parameter to all BNs in the bn_list
                for i in range(len(module.bn_list)):
                    for j in range(len(module.bn_list[i])):
                        new_key = f"{base_name}.bn_list.{i}.{j}.{param_name}"
                        expanded_checkpoint_state[new_key] = value
            elif isinstance(module, SwithableBatchNorm) and hasattr(module, 'bn'):
                # Map to the single bn
    ...
    for key, value in expanded_checkpoint_state.items():
