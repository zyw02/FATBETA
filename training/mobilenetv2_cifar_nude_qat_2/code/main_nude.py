...
def main():
    ...
    # 6. DDP wrapping (moved later)
    # ... but we need dataloader first
    ...
    # 3. Warm-up forward (initializes LSQ step size 's' based on REAL data at correct bit-width)
    input_size = 32 if configs.dataloader.dataset in ["cifar10", "cifar100"] else 224
    with torch.no_grad():
        # 🌟 CRITICAL: Use REAL DATA for warm-up to ensure 
        # LsqQuan 's' is initialized to meaningful values for CIFAR
        try:
            # Get one batch of real data
            warmup_inputs, _ = next(iter(train_loader))
            warmup_inputs = warmup_inputs.cuda()
            logger_info(logger, "[NUDE] Performing warm-up forward with real data batch...")
            model(warmup_inputs)
        except Exception as e:
            logger_info(logger, f"[NUDE] Warm-up with real data failed ({e}), falling back to random noise...")
            torch.manual_seed(0)
