...
def init_logger(experiment_name, output_dir, cfg_file=None):
    ...
    # 方法2：强制替换反斜杠为正斜杠
    log_file_str = str(log_file).replace("\\", "/")  # 将 \ 替换为 /
    # logging.config.fileConfig(cfg_file, defaults={'logfilename': str(log_file)})
    logging.config.fileConfig(cfg_file, defaults={'logfilename': str(log_file_str)})
    # logging.config.fileConfig(cfg_file, defaults={'logfilename': f'"{log_file_str}"'})
