...
def project_bfat_gradients(clean_grads, bfat_grads):
    """
    BFAT 梯度投影：如果 BFAT 梯度与 Clean 梯度夹角 > 90度（点积为负），
    则剔除 BFAT 梯度中对 Clean 梯度有害的分量。
    """
    ...
    for p, g_b in bfat_grads.items():
        if p in clean_grads:
            ...
            if dot_product < 0:
                ...
                projected_bfat_grads[p] = g_b - projection
            else:
...
def train(train_loader, model, criterion, optimizer, epoch, monitors, configs, model_ema=None, nr_random_sample=2, mode='training', soft_criterion=None, teacher_model=None, optimizer_q=None, annealing_schedule=None, freezing_annealing_schedule=None, IDM_weight=0.01, scaler=None, fault_injector=None, output_corrector=None, corrector_optimizer=None, device=None):
    ...
    if use_bfat:
        ...
        if target_module is not None:
            ...
            logger_info(logger, f'   Loss: (1 - cos_sim) * {bfat_weight}')
            ...
            logger_info(logger, f'⚠️ BFAT WARNING: Target layer {target_layer_name} not found!')
            use_bfat = False
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if use_bfat and fault_injector is not None and clean_feature is not None:
            # 记录 NR 累积的梯度并清空
            for p in model.parameters():
                if p.requires_grad and p.grad is not None:
            ...
            # 设置 BFAT 专用参数：仅翻转 MSB，指定 BER
            fault_injector.only_msb = True
            fault_injector.skip_msb = False
            fault_injector.bfat_bit_index = None  # 确保不使用绝对索引
            ...
            # 3. 计算相似度损失: L_bfat = 1 - cos_sim(f_clean, f_faulted)
                ...
                for p in model.parameters():
                    if p.requires_grad:
                        ...
                        if g_clean is not None and g_nr is not None:
                            ...
                            base_grads[p] = g_nr
                projected_bfat = project_bfat_gradients(base_grads, bfat_grads)
            else:
                # 默认：仅使用 clean_grads 作为投影基准
                projected_bfat = project_bfat_gradients(clean_grads, bfat_grads)
