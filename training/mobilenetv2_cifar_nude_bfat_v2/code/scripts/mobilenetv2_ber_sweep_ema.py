...
parser.add_argument('--ckpt', type=str, default="/workspace/FATBETA/training/mobilenetv2_cifar_nude_qat_2/mobilenetv2_cifar_nude_qat_2_checkpoint.pth.tar")
...
print(f"{C_BOLD}{C_TITLE}MobileNetV2 W{bits}A{bits} EMA BER Sweep - {ckpt_name}{C_RESET}")
...
for ber in bers:
    base_cmd = f"python {tool_path} --config {config} --ckpt {ckpt} --use_ema --ber {ber} --skip_baseline --dynamic_bits {bits}{seed_arg}"
