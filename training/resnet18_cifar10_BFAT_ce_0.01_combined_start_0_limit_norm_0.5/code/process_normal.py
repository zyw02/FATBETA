...
def project_bfat_gradients(clean_grads, bfat_grads, limit_norm=False, norm_ratio=0.5):
    """
    BFAT 梯度投影与模长限制：
    1. 方向投影：如果 BFAT 梯度与基准梯度夹角 > 90度（点积为负），则剔除有害分量。
    2. 模长限制：如果启用，确保 BFAT 梯度模长不超过基准梯度模长的一定比例。
    """
    ...
    for p, g_b in bfat_grads.items():
        if p in clean_grads:
            ...
            # --- 1. 方向修正 (Direction Correction) ---
            dot_product = torch.sum(g_c * g_b)
            if dot_product < 0:
                # 存在冲突，进行投影，删除有害分量
                norm_sq = torch.sum(g_c * g_c) + 1e-8
                projection = (dot_product / norm_sq) * g_c
                g_b_cleaned = g_b - projection
            else:
                g_b_cleaned = g_b
            
            # --- 2. 模长限制 (Magnitude Limiting) ---
            if limit_norm:
                norm_c = torch.norm(g_c)
                norm_b = torch.norm(g_b_cleaned)
                
                # 容错性更新的力度，不要超过主任务力度的 norm_ratio 倍
                target_norm = norm_c * norm_ratio
                
                if norm_b > target_norm:
                    scale = target_norm / (norm_b + 1e-8)
...
def train(train_loader, model, criterion, optimizer, epoch, monitors, configs, model_ema=None, nr_random_sample=2, mode='training', soft_criterion=None, teacher_model=None, optimizer_q=None, annealing_schedule=None, freezing_annealing_schedule=None, IDM_weight=0.01, scaler=None, fault_injector=None, output_corrector=None, corrector_optimizer=None, device=None):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if bfat_can_run:
            ...
            # --- 方案一：将所有 BN 层设为 eval 模式，防止故障特征污染统计量 ---
            # 由于模型使用了 SwithableBatchNorm，我们需要确保其内部所有的子 BN 层都不更新统计量
                ...
                for p in model.parameters():
                    if p.requires_grad:
                        ...
                        if g_clean is not None and g_nr is not None:
                            ...
                            base_grads[p] = g_nr
                projected_bfat = project_bfat_gradients(base_grads, bfat_grads, limit_norm=limit_norm, norm_ratio=norm_ratio)
            else:
                # 默认：仅使用 clean_grads 作为投影基准
                projected_bfat = project_bfat_gradients(clean_grads, bfat_grads, limit_norm=limit_norm, norm_ratio=norm_ratio)
