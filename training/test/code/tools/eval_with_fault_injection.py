...
def main():
    ...
    # 初始化输出修正器（如果checkpoint中包含）
    output_corrector = None
    num_classes = configs.dataloader.num_classes
    try:
        # 尝试从checkpoint加载corrector
        checkpoint_temp = torch.load(checkpoint_path, map_location='cpu') if checkpoint_path and os.path.exists(checkpoint_path) else None
        if checkpoint_temp and 'output_corrector' in checkpoint_temp:
            corrector_state = checkpoint_temp['output_corrector']
            num_layers = 8  # AlexNet约8层
            
            # 检测corrector版本
            if 'gap_mean' in corrector_state:
                logger_info(logger, "Detected V9 corrector (direction+magnitude, BER-aware)")
                output_corrector = create_output_corrector(
                    ...
                    wmid_tau=2.0,
                    wmid_beta=2.0,
                    logits_feature_dim=32,
                    fusion_hidden_dim=128,
                    gap_stats_momentum=0.95,
                    direction_deadzone=0.05,
                    anchor_ber=2e-2
                )
            elif 'logits_feature_extractor.0.weight' in corrector_state:
                # V8: 有logits_feature_extractor
                logger_info(logger, "Detected V8 corrector (logits statistics + self-supervised)")
                output_corrector = create_output_corrector(
                    num_classes=num_classes,
                    num_layers=num_layers,
                    device=device,
            ...
            elif 'prototypes' in corrector_state:
                # V7: 有prototypes
                logger_info(logger, "Detected V7 corrector (fault prototype learning)")
                num_prototypes = corrector_state['prototypes'].shape[0] if 'prototypes' in corrector_state else 10
                output_corrector = create_output_corrector(
                    num_classes=num_classes,
                    num_layers=num_layers,
                    device=device,
            ...
            elif 'K_p' in corrector_state:
                # V6或更早版本（B+A混合方案）
                logger_info(logger, "Detected V6 or earlier corrector (B+A hybrid)")
                output_corrector = create_output_corrector(
                    num_classes=num_classes,
                    num_layers=num_layers,
                    device=device,
                ...
                # 旧版本（尝试兼容）
                logger.warning("Unknown corrector version, trying default V8")
                output_corrector = create_output_corrector(
                    num_classes=num_classes,
                    num_layers=num_layers,
                    device=device,
            ...
            output_corrector.load_state_dict(corrector_state, strict=False)
            output_corrector.set_runtime_context(ber=0.0, stage='eval')
            logger_info(logger, f"✓ Output corrector loaded from checkpoint ({output_corrector.get_num_parameters()} parameters)")
        del checkpoint_temp
    except Exception as e:
        logger.warning(f"Could not load output corrector from checkpoint: {e}")
        import traceback
    ...
    # ========== 测试集采样校准（如果启用corrector） ==========
    if output_corrector is not None and args.calibration_samples > 0:
        logger_info(logger, "\n" + "="*60)
        logger_info(logger, "Calibrating Corrector from Test Set Samples")
        logger_info(logger, "="*60)
        logger_info(logger, f"Sampling {args.calibration_samples} samples from test set for calibration")
        logger_info(logger, f"(~{args.calibration_samples / 100:.1f}% of CIFAR10 test set, prevents data leakage)")
        # 从测试集采样指定数量样本进行校准，避免数据泄漏
        ...
        logger_info(logger, "✓ Corrector calibration completed\n")
    elif output_corrector is not None:
        logger_info(logger, "⚠️  Corrector calibration disabled (--calibration_samples=0)")
        logger_info(logger, "Using baseline statistics from training checkpoint\n")
    ...
    # validate返回(model_accs, corrected_accs)或model_accs
    if isinstance(baseline_result, tuple) and len(baseline_result) == 2:
        baseline_acc, baseline_acc_corrected = baseline_result
        baseline_acc = baseline_acc[0] if isinstance(baseline_acc, list) else baseline_acc
        baseline_acc_corrected = baseline_acc_corrected[0] if isinstance(baseline_acc_corrected, list) else baseline_acc_corrected
        logger_info(logger, f"Baseline Top-1 Accuracy: {baseline_acc:.2f}% (Model) | {baseline_acc_corrected:.2f}% (With Corrector)")
    else:
    ...
    for idx, ber in enumerate(ber_values, 1):
        ...
        for trial_idx in range(args.num_trials):
            ...
            injector = FaultInjector(
                ...
                skip_first_last=True,  # Skip first and last layers, only inject faults in middle layers
            )
