...
def main():
    ...
    if configs.eval:
        ...
        for epoch in range(start_epoch, configs.epochs):
            ...
            print(f'time: {time.strftime("%Y-%m-%d %H:%M:%S")}, top1_acc: {t_top1}, top5_acc: {t_top5}, loss: {t_loss}')
            ...
            if epoch % 20 == 0 or epoch == configs.epochs-1:
                torch.save(model.state_dict(),f'trained_models/alexnet_no_freezing_seed19_w6a6_{configs.epochs-1}.pt')
            # if epoch % 20 == 0:
            #     save_checkpoint(epoch, configs.arch, model, target_model, optimizer, {
