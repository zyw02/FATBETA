...
def main():
    ...
    # 打印故障注入统计信息
    if (not args.skip_fault) and injector_binary.enable_statistics:
    ...
    if accuracy_binary is not None:
        print(f"故障注入后准确率: {accuracy_binary:.2f}%")
    if accuracy_baseline is not None and accuracy_binary is not None:
