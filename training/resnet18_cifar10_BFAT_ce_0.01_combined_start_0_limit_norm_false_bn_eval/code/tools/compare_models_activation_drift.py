...
def load_and_process_data(csv_path):
    ...
    l2_relative = df['l2_relative'].iloc[0]
    cosine_similarity = df['cosine_similarity'].iloc[0]

    # Parse layer-wise metrics
    import ast
    l2_by_layer = ast.literal_eval(df['l2_by_layer'].iloc[0])
    cosine_by_layer = ast.literal_eval(df['cosine_by_layer'].iloc[0])

    return {
        'l2_relative': l2_relative,
        'cosine_similarity': cosine_similarity,
        'l2_by_layer': l2_by_layer,
        'cosine_by_layer': cosine_by_layer
    }
