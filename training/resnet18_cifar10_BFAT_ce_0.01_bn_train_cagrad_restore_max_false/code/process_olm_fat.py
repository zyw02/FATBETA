...
def show_training_info(meters, target_bits, nr_random_sample, mode):
    ...
    for i in range(iters):
        # 只在Max Training（第一个meter，i==0）显示corrupted准确率
        if i == 0 and 'corrupted_top1' in meters[i] and meters[i]['corrupted_top1'].count > 0:
            logger.info('==> %s Top1: %.3f    Top5: %.3f    Corrupted-Top1: %.3f    Corrupted-Top5: %.3f    Loss: %.3f', 
                        meters[i]['name'],
                        meters[i]['top1'].avg, meters[i]['top5'].avg,
                        meters[i]['corrupted_top1'].avg, meters[i]['corrupted_top5'].avg,
                        meters[i]['loss'].avg)
        else:
...
def get_meters(mode, target_bits, nr_random_sample, sample_current_max, sample_current_min):
    if mode == 'training':
        ...
        meters = [{
            ...
            'top1': AverageMeter(),
            'top5': AverageMeter(),
            'corrupted_top1': AverageMeter(),  # 故障注入后的Top1准确率
            'corrupted_top5': AverageMeter(),  # 故障注入后的Top5准确率
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if sample_current_max:
            ...
            if use_fault_aware_training and fault_injector is not None and search_olm_manager is not None and epoch >= fat_start_epoch:
                ...
                # 计算故障注入后的准确率（corrupted accuracy）
                corrupted_acc1, corrupted_acc5 = accuracy(outputs_faulted.data, targets.data, topk=(1, 5))
                meters[0]['corrupted_top1'].update(corrupted_acc1.item(), inputs.size(0))
                meters[0]['corrupted_top5'].update(corrupted_acc5.item(), inputs.size(0))
            else:
                # 无故障注入的标准训练
