...
class SchedulerConfig:
    ...
    def __init__(self, sched, epochs, min_lr, warmup_lr, warmup_epochs, cooldown_epochs, decay_rate):
        self.sched = sched
        self.epochs = epochs
        self.min_lr = min_lr
...
def main():
    ...
    # --- 2.1. Set correct bit-width for the model ---
    logger.info("Setting model to highest bit-width to ensure correct clean accuracy...")
    if hasattr(configs, 'target_bits') and configs.target_bits:
        max_bit = max(configs.target_bits)  # Get the highest bit-width e.g., from [6, 5, 4, 3, 2]
        # Execute a dummy forward pass to initialize output_size for all layers
        training_model.eval()
        dummy_input = torch.randn(1, 3, 32, 32).to(device)
        with torch.no_grad():
            _ = training_model(dummy_input)
        # Use sample_max_cands to set model to maximum bit-width (same as "Mixed 0" in training)
        from util.mpq import sample_max_cands
        sample_max_cands(training_model, configs)
        logger.info(f"Model bit-width set to maximum ({max_bit}-bit) using sample_max_cands.")
    else:
        logger.warning("`target_bits` not found in config. Skipping manual bit-width setting.")
    ...
    # Hooks are now registered in the constructor, so this call is not needed.
    # collector_for_dim._register_hooks()
    dummy_input = torch.randn(2, 3, 32, 32).to(device)
    training_model(dummy_input)
    # The first return value is a list of tensors, the second is a list of integers (feature dimensions)
    _, feature_dims_per_layer = collector_for_dim.build_layer_features(device)
    ...
    import copy
    restorer_configs = copy.deepcopy(configs)
    restorer_configs.lr = configs.sensitive_restorer.restorer_lr
    restorer_configs.epochs = configs.sensitive_restorer.stage2_epochs
    # Faster LR adaptation to plateau for restorer
    restorer_configs.sched = 'plateau'
    restorer_configs.decay_rate = getattr(configs, 'decay_rate', 0.5)
    restorer_configs.min_lr = getattr(configs, 'min_lr', 1e-6)
    restorer_configs.warmup_lr = 0.0
    restorer_configs.warmup_epochs = 0
    restorer_configs.cooldown_epochs = 0
    # Plateau-specific knobs
    restorer_configs.patience_t = getattr(configs, 'patience_t', 2)
    restorer_configs.threshold = getattr(configs, 'threshold', 5e-4)
    restorer_configs.mode = 'min'

    restorer_optimizer, _, restorer_lr_scheduler, _ = create_optimizer_and_lr_scheduler(
        restorer, restorer_configs
    )
    ...
    # Create and attach the collector to the restorer before starting the training loop
    collector = SensitiveActivationCollector(
        model=training_model,
        sensitive_info=sensitive_channels,
        baseline_stats=baseline_stats
    )
    restorer.collector = collector
    logger.info("Activation collector attached to the restorer.")
           
    for epoch in range(configs.sensitive_restorer.stage2_epochs):
        if is_master():
            logger.info('>>>>>>>> Epoch %3d' % epoch)

        # Note: BER will be randomly sampled for each batch in train_process
        # No need to set it here at epoch level
        if is_master():
            logger.info(f"[Stage2] Using random BER sampling [1e-2, 1e-1] for each batch at epoch {epoch}")

        # Call the training process function
        rest_acc, _, train_loss = train_process(
            train_loader=train_loader,
            model=training_model,
            criterion=criterion,
        ...
        # Compute validation metric for Plateau scheduler
        metric_for_scheduler = float(train_loss)
        logger.info(f"Epoch {epoch}: Train Loss (scheduler metric) = {metric_for_scheduler:.4f}")
        
        if restorer_lr_scheduler is not None:
            restorer_lr_scheduler.step(epoch, metric_for_scheduler)
        ...
        if is_master():
            save_checkpoint(
                ...
                arch=configs.arch,
                model=restorer,
                optimizer=restorer_optimizer,
                lr_scheduler=restorer_lr_scheduler,
                is_best=False,
                name=f"{configs.name}_restorer",
                output_dir=log_dir,
                # Pass dummy values for unused parameters to match function signature
                target_model=None,  # Set to None as Restorer has no EMA
                lr_scheduler_q=None,
                optimizer_q=None
            )
...
def validate_restorer(val_loader, model, criterion, restorer, fault_injector, device, configs):
    model.eval()
    restorer.eval()
    val_loss_meter = AverageMeter()
    
    # Set model to maximum bit-width for validation
    from util.mpq import sample_max_cands
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(val_loader):
            ...
            logits_clean = model(inputs)  # Not needed for val_loss, but keep if useful
            
            if fault_injector: fault_injector.enable()
            
            # Add collector logic here
            collector = getattr(restorer, 'collector', None)
            if collector:
                collector.clear()
            ...
            logits_restored, _ = restorer(logits_faulted, layer_features)
            
            ce_loss = criterion(logits_restored, targets)
            val_loss_meter.update(ce_loss.item(), inputs.size(0))
            
            # Limit validation to a few batches for speed
            if batch_idx >= 20:
                break
    
    # Return average loss, or a default value if no batches were processed
    return val_loss_meter.avg if val_loss_meter.count > 0 else 2.3026
