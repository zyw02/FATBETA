...
if os.name == 'posix' and sys.platform.startswith('linux'):
    sys.path.append('/root/autodl-tmp/quansam/')
...
from process import train, validate, PerformanceScoreboard, alexnet_train, alex_train_2
from evolution_search import EvolutionSearcher
from data.cifar import Cifar100, Cifar10
from torch.optim.lr_scheduler import CosineAnnealingLR
import argparse
from bsam import BSAM
from scheduler import ProportionScheduler
from train_epoch_bsam import train_with_bsam_one_epoch
from scalablewp.qbsam import QBSAM
...
def main():
    ...
    base_optimizer, optimizer_q, lr_scheduler, lr_scheduler_q = create_optimizer_and_lr_scheduler(
        model, configs)



    # 控制变量: grad_clip, optimizer_q, freezing_layer
    # bsam起始段
    ...
    parser.add_argument("--rho_max_sharp", default=0.01, type=int, help="Rho parameter for SAM.")
    parser.add_argument("--rho_min_sharp_max", default=0.001, type=int, help="")
    ...
    #
    # scheduler = CosineAnnealingLR(base_optimizer, T_max=args.epochs * len(dataset.train), eta_min=1e-5)
    rho_scheduler = ProportionScheduler(pytorch_lr_scheduler=lr_scheduler, max_lr=args.learning_rate, min_lr=0.0,
                                        max_value=args.rho_min_sharp_max, min_value=args.rho_min_sharp_min)
    #
    # optimizer = BSAM(base_optimizer.param_groups, base_optimizer, rho_max_sharp=args.rho_max_sharp,
    #                  rho_min_sharp=args.rho_min_sharp_max, rho_scheduler=rho_scheduler,
    #                  adaptive=args.adaptive, lr=args.learning_rate)
    optimizer = QBSAM(base_optimizer.param_groups, model, base_optimizer, rho_max_sharp=args.rho_max_sharp,
                     rho_min_sharp=args.rho_min_sharp_max, rho_scheduler=rho_scheduler,
                     adaptive=args.adaptive, lr=args.learning_rate)
    ...
    if configs.resume.path and os.path.exists(configs.resume.path):
        model, start_epoch, _ = load_checkpoint(model, configs.resume.path, 'cuda', lean=configs.resume.lean,
                                                optimizer=optimizer, override_optim=configs.eval,
                                                lr_scheduler=lr_scheduler, lr_scheduler_q=lr_scheduler_q,
    ...
    if configs.eval:
        ...
        if searcher == 'evolution_searcher':
            ...
            searcher = EvolutionSearcher(configs, 'cuda', train_loader, target_model.ema, val_loader, test_loader,
            ...
            conf = search(loader=train_loader, model=target_model.ema, criterion=criterion,
            ...
            conf = do_random_search(train_loader, model, criterion=criterion, metrics=configs.bops_limits,
        ...
        if 1:
            ...
            for epoch in range(start_epoch, configs.epochs):
                ...
                train_with_bsam_one_epoch(model, train_loader, test_loader, criterion, optimizer, epoch, configs,
                                          lr_scheduler=lr_scheduler, optimizer_q=optimizer_q, annealing_schedule=annealing_schedule,
                ...
                #              )
                if lr_scheduler is not None:
                    lr_scheduler.step(epoch + 1) # 调整学习率

                if lr_scheduler_q is not None:
                    lr_scheduler_q.step() # 调整学习率

                # scheduler.step()
                if (epoch + 1) % 20 == 0 and epoch > 179:
                    torch.save(model.state_dict(), f'../trained_models/alexnet_bsam_restart_{args.rho_max_sharp}_{args.rho_min_sharp_max}.pt')
