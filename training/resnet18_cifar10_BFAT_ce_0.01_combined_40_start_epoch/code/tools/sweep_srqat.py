...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional lambda values for orthogonality_penalty.lambda_ortho (strings). "
            "If provided, generated configs will sweep these values (and can form a grid with --lambdas)."
        ),
    )
    parser.add_argument(
        "--out_config_dir",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If provided, run fault injection multiple times per BER with these seeds and "
            "write mean/std to CSV. If omitted, uses --seed (single run per BER)."
        ),
    )
    parser.add_argument(
        "--baseline_seed",
        type=int,
        default=None,
        help="Seed used for baseline evaluation (default: first of --eval_seeds or --seed).",
    )
    parser.add_argument(
        "--csv",
        type=str,
    ...
    generated: List[Path] = []
    ortho_lams = args.ortho_lambdas if args.ortho_lambdas is not None else [None]
    for lam_str in args.lambdas:
        for ortho_str in ortho_lams:
            cfg = deepcopy(base_cfg)

            # --- scale penalty (SR-QAT) ---
            ls_tag = _sanitize_lambda_tag(lam_str)
            cfg.setdefault("scale_penalty", {})
            cfg["scale_penalty"]["enabled"] = True
            cfg["scale_penalty"]["lambda_scale"] = float(lam_str)

            # --- orthogonality penalty (optional sweep) ---
            if ortho_str is not None:
                lo_tag = _sanitize_lambda_tag(str(ortho_str))
                cfg.setdefault("orthogonality_penalty", {})
                cfg["orthogonality_penalty"]["enabled"] = True
                cfg["orthogonality_penalty"]["lambda_ortho"] = float(ortho_str)
                cfg["name"] = f"{base_name}_ls{ls_tag}_lo{lo_tag}"
            else:
                cfg["name"] = f"{base_name}_ls{ls_tag}"
    ...
    if args.eval:
        eval_seeds: List[int] = list(args.eval_seeds) if args.eval_seeds is not None else [int(args.seed)]
        baseline_seed = int(args.baseline_seed) if args.baseline_seed is not None else int(eval_seeds[0])
        ...
        with csv_path.open("w", encoding="utf-8", newline="") as f:
            ...
            for cfg_path in generated:
                ...
                # Baseline: run once per checkpoint to avoid "baseline jumps" caused by re-running
                # evaluation across different FI seeds / CUDA nondeterminism.
                print(f"\n[SRQAT][BASELINE] {cfg['name']} | lambda={lam} | seed={baseline_seed}")
                out_b = _run_capture(
                    [
                        "python3",
                ...
                baseline_acc, _ = _parse_fault_injection_output(out_b)
                if baseline_acc is None:
                    raise RuntimeError(
                        "Failed to parse baseline accuracy from baseline-only eval output.\n"
                        f"exp={cfg['name']} seed={baseline_seed}\n\n{out_b}"
                    )
                baseline_mean = float(baseline_acc)
                baseline_std = 0.0
                ...
                for ber_str in args.bers:
                    ...
                    print(
                        ...
                        f"runs={len(eval_seeds)} seeds={eval_seeds}"
                    )

                    fault_list: List[float] = []
                    for s in eval_seeds:
                        out = _run_capture(
                            [
                                "python3",
                        ...
                        if fault_acc is None:
                            raise RuntimeError(
                                "Failed to parse fault accuracy from eval output.\n"
                                f"exp={cfg['name']} ber={ber_str} seed={s}\n\n{out}"
                            )
                        fault_list.append(float(fault_acc))

                    # Mean/std (std=0 if only one run)
                    if len(fault_list) == 1:
                        fault_mean = fault_list[0]
                        fault_std = 0.0
                    else:
                        fault_mean = statistics.mean(fault_list)
                        fault_std = statistics.stdev(fault_list)
                    writer.writerow(
                        {
                            "exp_name": cfg["name"],
                            "lambda_scale": lam,
                            "ber": ber,
                            # Backward-compatible columns: now store mean±std as a string.
                            # Example: "88.38±0.00"
                            "baseline_acc": f"{baseline_mean:.4g}±{baseline_std:.3g}",
                            "fault_acc": f"{fault_mean:.4g}±{fault_std:.3g}",
