...
class LayerwiseRestorer(nn.Module):
    ...
    def __init__(self, num_layers: int, feature_dims_per_layer: List[int], num_classes: int, expert_hidden_dim: int):
        ...
        for feature_dim in feature_dims_per_layer:
            ...
            gate = nn.Sequential(
                nn.Linear(feature_dim, max(8, expert_hidden_dim // 16)),
                nn.ReLU(inplace=True),
                nn.Linear(max(8, expert_hidden_dim // 16), 1)
            )
    ...
    def forward(self, logits, layer_features: List[torch.Tensor]):
        ...
        stacked_deltas = torch.stack(all_deltas, dim=0)
        stacked_weights = torch.stack(all_weights, dim=0)
        weight_logits = stacked_weights.squeeze(-1)
        weight_norm = torch.softmax(weight_logits, dim=0)
        weight_expanded = weight_norm.unsqueeze(-1).expand_as(stacked_deltas)
        mean_delta = torch.sum(stacked_deltas * weight_expanded, dim=0)
