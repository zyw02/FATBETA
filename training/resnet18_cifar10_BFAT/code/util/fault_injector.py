...
class FaultInjector:
    ...
    def __init__(
        ...
        skip_msb: bool = False,  # 是否跳过最高位（MSB）故障注入
        only_msb: bool = False,  # 是否只对最高位（MSB）进行故障注入
    ) -> None:
        self.model = model
        self.mode = mode
        self.ber = ber
        ...
        self.skip_msb = skip_msb  # 是否跳过最高位故障注入
        self.only_msb = only_msb  # 是否只对最高位进行故障注入
    ...
    def _process_pending_statistics(self) -> None:
        ...
        for idx, (_, total_bits, total_params, stats_key, __) in enumerate(self._pending_stats):
            ...
            # 调试信息：打印每个stats_key
            import sys
            print(f"[DEBUG] _process_pending_statistics: 处理 stats_key='{stats_key}', flipped_bits={flipped_bits_count}, total_bits={total_bits}, total_params={total_params}", file=sys.stderr, flush=True)
    ...
    def _wrap_modules(self) -> None:
        ...
        for name, module in all_quantized_layers:
            ...
            # If exclude_layers is provided, skip excluded layers
            if self.exclude_layers and name in self.exclude_layers:
                continue
    ...
    def _inject_on_quantized_tensor(
        ...
        if use_gray_code:
            ...
            # [DEBUG] 检查第950行是否被执行
            import sys
            # print(f"[DEBUG line 950] 执行OLM编码查找表索引操作", file=sys.stderr, flush=True)
            # print(f"[DEBUG line 950] code_shifted类型: {code_shifted.dtype}, 形状: {code_shifted.shape}, 设备: {code_shifted.device}", file=sys.stderr, flush=True)
            # print(f"[DEBUG line 950] code_shifted.clamp(0, n_levels)类型: {code_shifted.clamp(0, n_levels).dtype}", file=sys.stderr, flush=True)
            # print(f"[DEBUG line 950] lookup_table类型: {lookup_table.dtype}, 形状: {lookup_table.shape}, 设备: {lookup_table.device}", file=sys.stderr, flush=True)
            # 修复bug: 将float类型转换为long类型后再索引
            code = lookup_table[code_shifted.clamp(0, n_levels).long()]
            # print(f"[DEBUG line 950] 索引操作成功！code类型: {code.dtype}, 形状: {code.shape}", file=sys.stderr, flush=True)
        ...
        try:
            ...
            import sys
            print(f"[ERROR] 在fault_injector.py:1003处发生异常: {type(e).__name__}: {e}", file=sys.stderr, flush=True)
            print(f"[ERROR] flat_faulted.shape={flat_faulted.shape}, flat_faulted.dtype={flat_faulted.dtype}, flat_faulted.device={flat_faulted.device}", file=sys.stderr, flush=True)
            print(f"[ERROR] n_levels={n_levels}, code_dtype={code_dtype}, device={device}", file=sys.stderr, flush=True)
            if flat_faulted.numel() > 0:
                print(f"[ERROR] flat_faulted.min()={flat_faulted.min().item()}, flat_faulted.max()={flat_faulted.max().item()}", file=sys.stderr, flush=True)
            import traceback
        ...
        if use_gray_code:
            ...
            # 向量化查找
            # 修复bug: 将int16类型转换为long类型后再索引
            # flat_faulted = reverse_lookup[flat_faulted.clamp(0, max_code).long()].clamp(0, n_levels)
            # print(f"flat_faulted.clamp(0,max_code).dtype:{flat_faulted.clamp(0,max_code).dtype}")
            flat_faulted = reverse_lookup[flat_faulted.clamp(0, max_code).long()].clamp(0, n_levels)
    ...
    def _generate_flip_mask(self, N: int, k: int, device: torch.device, layer_name: Optional[str] = None, mask_seed: Optional[int] = None) -> torch.Tensor:
        ...
        if self.mode == "ber":
            ...
            # Handle MSB-related options (skip_msb and only_msb are mutually exclusive)
            if self.skip_msb and self.only_msb:
                raise ValueError("skip_msb and only_msb cannot be both True")

            if self.bfat_bit_index is not None:
                # BFAT 模式：仅在指定的 bit_index 上应用概率 p
                p_tensor = torch.zeros((k,), dtype=torch.float32, device=device)
                # 如果 bit_index 超过当前层位宽，自动调整到该层的最高位
                target_idx = min(self.bfat_bit_index, k - 1)
                p_tensor[target_idx] = p
            elif self.skip_msb:
                # Skip MSB: create probability tensor [p, p, ..., p, 0] where MSB has p=0
                p_tensor = torch.full((k,), p, dtype=torch.float32, device=device)
                p_tensor[-1] = 0.0  # Set MSB (highest bit) probability to 0
            elif self.only_msb:
                # Only MSB: create probability tensor [0, 0, ..., 0, p] where only MSB has p
                p_tensor = torch.zeros((k,), dtype=torch.float32, device=device)
                p_tensor[-1] = p  # Set only MSB (highest bit) probability to p
            else:
                p_tensor = p
            ...
            if layer_name is not None and self.use_position_based_mask:
                ...
                # Correct comparison: p_tensor might be a vector or scalar
                mask_flat = hash_tensor < (p_tensor.repeat(N) if isinstance(p_tensor, torch.Tensor) else p_tensor)
                ...
                # p_tensor is already correctly set as scalar or vector
                return rand_tensor < p_tensor
        else:
