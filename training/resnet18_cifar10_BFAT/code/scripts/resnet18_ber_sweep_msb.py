...
ckpt = "/root/autodl-tmp/retraining-free-quantization/training/resnet18_cifar10_baseline/resnet18_cifar10_baseline_checkpoint.pth.tar"
config = "configs/training/train_resnet18_cifar10_single_gpu.yaml"

print(f"ResNet18 W4A4 BER Sweep - Baseline Checkpoint")
print(f"{'BER':<10} | {'All Bits Acc':<15} | {'Skip MSB Acc':<15} | {'Only MSB Acc':<15}")
print("-" * 65)

for ber in bers:
    # 1. 跑 All Bits (w4a4)
    cmd_all = f"python tools/test_fault_injection_baseline_resnet18.py --config {config} --ckpt {ckpt} --ber {ber} --skip_baseline --dynamic_bits 4"
    ...
    # 2. 跑 Skip MSB (w4a4)
    cmd_skip = f"python tools/test_fault_injection_baseline_resnet18.py --config {config} --ckpt {ckpt} --ber {ber} --skip_msb --skip_baseline --dynamic_bits 4"
    ...
    # 3. 跑 Only MSB (w4a4)
    cmd_only = f"python tools/test_fault_injection_baseline_resnet18.py --config {config} --ckpt {ckpt} --ber {ber} --only_msb --skip_baseline --dynamic_bits 4"
