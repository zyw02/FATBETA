...
def main():
    ...
    if configs.resume.path and os.path.exists(configs.resume.path):
        ...
        # Reseting quantizer init_state for fresh LSQ initialization based on loaded weights
        if configs.resume.lean:
            logger_info(logger, "Reseting quantizer init_state for fresh LSQ initialization...")
            unwrapped_model = model.module if hasattr(model, 'module') else model
            for m in unwrapped_model.modules():
                if hasattr(m, 'quan_w_fn') and hasattr(m.quan_w_fn, 'init_state'):
                    m.quan_w_fn.init_state.fill_(0)
                if hasattr(m, 'quan_a_fn') and hasattr(m.quan_a_fn, 'init_state'):
                    m.quan_a_fn.init_state.fill_(0)
