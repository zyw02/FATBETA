...
def compute_activation_metrics(clean_acts, faulted_acts):
    ...
    metrics['l2_relative'] = np.mean(l2_distances)
    metrics['cosine_similarity'] = np.mean(cosine_sims)
    metrics['l2_by_layer'] = dict(zip(clean_acts.keys(), l2_distances))
    metrics['cosine_by_layer'] = dict(zip(clean_acts.keys(), cosine_sims))
